...
export default class Web3IndexerService {
  ...
  constructor(web3Config: Web3Config) {
    ...
    this._indexerSmartContract = new this._web3.eth.Contract(
      web3Config.indexerabi,
      web3Config.indexeraddress
    );
    this._jnsSmartContract = new this._web3.eth.Contract(
      web3Config.jnsabi,
      web3Config.jnsaddress
    );
  }
  ...
  public async ListByTags(
    ...
    const result = await this._indexerSmartContract.methods
      .getWebSite(tags, pageNumber, pageSize)
      .call()
      .then(a => a);
    ...
    for (let i = 0; i < result[0].length; i++) {
      ...
      if (element && element.length > 0) {
        ...
        const jns = await this._jnsSmartContract.methods
          .getDomainByStorageHash(storageHash)
          .call()
          .then(a => a);
